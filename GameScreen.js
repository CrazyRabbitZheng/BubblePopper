...
export default function GameScreen() {
   ...
   */
    ...
    //Assign a random color when spawning bubbles.
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) {
          ...
          setGameOver(true);
          // When time is up, clear the bubbles left on screen. setBubbles() calls an empty array []
          setBubbles([]);
          return 0;
        }
        return prev - 1;
  ...
  return (
    <View style={styles.container}>
      {/* Game area */}
      <TouchableWithoutFeedback onPress={handleTap} disabled={!gameStarted || gameOver}>{/* I wrapped the gameArea with ImageBackground component to apply to the whole screen a background page. */}
        <ImageBackground
            source={backgroundImg}
            style={styles.gameArea}
            resizeMode='cover'
            >
            <View style={{ flex: 1}}>{/* This is to wrap everything inside one View. */}
          {/* Bubbles */}
          {bubbles.map(bubble => (
            <Bubble
